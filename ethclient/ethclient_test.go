...
// Verify that Client implements the metrachain interfaces.
var (
	_ = metrachain.ChainReader(&Client{})
	_ = metrachain.TransactionReader(&Client{})
	_ = metrachain.ChainStateReader(&Client{})
	_ = metrachain.ChainSyncReader(&Client{})
	_ = metrachain.ContractCaller(&Client{})
	_ = metrachain.GasEstimator(&Client{})
	_ = metrachain.GasPricer(&Client{})
	_ = metrachain.LogFilterer(&Client{})
	_ = metrachain.PendingStateReader(&Client{})
	// _ = metrachain.PendingStateEventer(&Client{})
	_ = metrachain.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new metrachain service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == metrachain.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != metrachain.NotFound {
		t.Fatal("error should be metrachain.NotFound")
	}
}
