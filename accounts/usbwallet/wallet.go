...
type wallet struct {
	...
	deriveChain     metrachain.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain metrachain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
